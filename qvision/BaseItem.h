...
struct MEllipse
{
	...
	MEllipse(qreal x, qreal y, qreal Lenth1, qreal Lenth2, qreal Pi)
	{
		row = y;
		col = x;
		phi = Pi;
...
struct MLine
{
	...
	MLine(qreal x1, qreal y1, qreal x2, qreal y2)
	{
		y1 = y1;
		x1 = x1;
		y2 = y2;
...
public:
    ...
    qreal PolygonLineWidth = 2;  //控制点初始尺寸
public slots:
...
//******多边形-2******
class MyPolygonItem : public PolygonItem
{
    Q_OBJECT

public:
    MyPolygonItem();
    void push(QPointF p, bool is_finish);
    void pop();
    void finish();
    QList<QPointF> getPts() const;
    QRect getRect() const;
    int getType() const;
protected:
    virtual QRectF boundingRect() const override;
...
public:
	...
	void GetEllipse(MEllipse& MRRect);
protected:
	virtual void paint(QPainter* painter,
		const QStyleOptionGraphicsItem* option,
		QWidget* widget) override;
