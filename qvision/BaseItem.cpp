...
void BaseItem::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
	...
	QFont font;
	font.setPointSizeF(12 / (*scale)); //字体大小
...
void PolygonItem::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
	...
	Q_UNUSED(option);
	Q_UNUSED(widget);
	thisPen.setWidthF(PolygonLineWidth);
	painter->setPen(thisPen);
    if (!center.isNull()){
        painter->drawEllipse(center, ContrSize, ContrSize);
    }
	if (Finished)
	{
...
MyPolygonItem::MyPolygonItem()
{
    ...
    setZValue(10);  // 设置 ZValue 高于 PixmapItem
}
QPointF MyPolygonItem::getCenter()
{
    m_pts.clear();
    QPolygonF polygon;
    for (int i = 1; i < ControlList.length(); i++)
    {
        m_pts << ControlList[i]->GetPoint();
        polygon << ControlList[i]->GetPoint();
    }
    qDebug() << m_pts;
...
QRect MyPolygonItem::getRect() const { return m_boundingRect.toRect(); }
QList<QPointF> MyPolygonItem::getPts() const { return m_pts; }
int MyPolygonItem::getType() const { return 9;}
void MyPolygonItem::finish() {
    Finished = true;
    center = getCenter();

    this->update();
}
void MyPolygonItem::push(QPointF p, bool is_finish)
{
    Finished = is_finish;

    auto tmp = new ControlItem(this, p, ControlList.length()+1);
    tmp->setVisible(true);
    ControlList << tmp;
...
void MyPolygonItem::pop()
{
    auto item = ControlList.last();
...
void EllipseItem::GetEllipse(MEllipse& MRRect)
{
	QPointF GetRRect = this->mapToScene(ControlList[0]->x(), ControlList[0]->y());
	MRRect.col = GetRRect.x();
	MRRect.row = GetRRect.y();
